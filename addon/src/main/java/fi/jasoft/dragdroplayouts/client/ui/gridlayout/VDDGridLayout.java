...
public class VDDGridLayout extends VGridLayout implements VHasDragMode,
    VDDHasDropHandler<VDDGridLayoutDropHandler>, DragStartListener, VHasDragFilter,
  ...
  /**
   * A helper class returned by getCellDetailsByCoordinates() which contains positional and size
   * data of the cell.
   */
  ...
  private CellDetails getCellDetailsByCoordinates(int x, int y) {
    ...
    // Sanity check
    if (cd.row == -1 || cd.column == -1 || cd.width == -1 || cd.height == -1) {
      return null;
    }
