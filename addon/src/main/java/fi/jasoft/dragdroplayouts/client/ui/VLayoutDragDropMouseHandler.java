...
public class VLayoutDragDropMouseHandler implements MouseDownHandler, TouchStartHandler,
  ...
  private final List<DragStartListener> dragStartListeners =
      new ArrayList<VLayoutDragDropMouseHandler.DragStartListener>();
  ...
  public interface DragStartListener {
     ...
     * 
     * @param widget The widget which is about to be dragged
     * @param mode The draggin mode
   ...
   * @param dragMode The drag mode of the layout
   */
  ...
  private boolean isMouseDragEvent(NativeEvent event) {
    boolean hasModifierKey =
        event.getAltKey() || event.getCtrlKey() || event.getMetaKey() || event.getShiftKey();
  ...
  protected void initiateDrag(NativeEvent event) {
    ...
    VTransferable transferable =
        VDragDropUtil.createLayoutTransferableFromMouseDown(event, root, target);
    ...
    if (target instanceof TabCaption) {
      ...
      ComponentConnector connector =
          (ComponentConnector) transferable.getData(Constants.TRANSFERABLE_DETAIL_COMPONENT);
    ...
    if (dragImageElement != null) {
       ...
       */
      Element rowElement =
          (Element) VDDFormLayout.getRowFromChildElement(
              (com.google.gwt.dom.client.Element) w.getElement().cast(),
              (com.google.gwt.dom.client.Element) root.getElement().cast()).cast();
          ...
          if (mouseUpHandlerReg != null) {
            ...
            if (currentDraggedWidget != null) {
              ...
              if (dragImageProvider != null) {
                com.google.gwt.dom.client.Element dragImageElement =
                    dragImageProvider.getDragImageElement(currentDraggedWidget);
                if (dragImageElement != null) {
   ...
   * 
   * @param dragMode The drag mode to use
   */
